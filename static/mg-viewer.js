...
let grammars = {
  ...
  "Stabler 2013 with head movement": [
    "C",
    [
      "::T= C",
      "::T= +W C",
...
function set_parse_tree(s) {
  ...
  const svg = d3
    .select("#parse-tree")
    .append("svg")
    .attr("viewBox", [x0 - dy, y0 - dx, width, height])
    .style("width", "100%")
    .style("height", "100%")
  ...
  const simulation = d3
    ...
    .force(
      ...
      d3
        ...
        .distance((d) => {
          return Math.sqrt(
            (d.source.targetX - d.target.targetX) ** 2 +
              (d.source.targetY - d.target.targetY) ** 2,
          );
        })
        .strength(1.5),
    )
  ...
  const move_details = tree_with_movement["phrasal_movement"].concat(
    tree_with_movement["head_movement"],
  );
  const movers = move_details.map((_) => {
  ...
  root.descendants().forEach((node) => {
    if (is_in_move.has(node.data.gorn)) {
      move_details.forEach((x, i) => {
        ...
        movers[i]["phrasal_movement"] =
          i < tree_with_movement["phrasal_movement"].length;
      });
    }
  });
